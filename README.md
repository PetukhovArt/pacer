...
[![Release](https://img.shields.io/github/v/release/PetukhovArt/pacer?style=flat-square&color=e8c547&label=release)](https://github.com/PetukhovArt/pacer/releases)
[![Build](https://img.shields.io/github/actions/workflow/status/PetukhovArt/pacer/release.yml?style=flat-square&label=build)](https://github.com/PetukhovArt/pacer/actions)
...
pacer replaces that with a tree and a color:
...
- **Every open pull request under them, readable in place.** Open a project and pacer asks `gh` what's
...
Afterwards, `pacer upgrade` runs that same script for you. It refuses to clobber a local `cargo build`
(pass `--force` if you mean it). Upgrading while a daemon is running is safe: sessions keep running on
the old binary until you `pacer kill` and relaunch.

> **Prerequisite:** at least one agent CLI on your `PATH` — `claude`, `codex`, or `cursor-agent`.
> pacer spawns them; it doesn't ship them.

## Quickstart

**1. Add a repo.** pacer is project-first, and a project is just a git checkout:

```sh
pacer add ~/code/my-app       # or, from inside the repo: pacer add .
```

**2. Open the TUI.** A bare `pacer` launches it and auto-starts the daemon:
...
"do this in a worktree" and it creates one through pacer and moves itself into it (see *How it works*).
...
leaving pacer; `g` opens its diff in the same viewer your worktree diffs use, `Enter` or a double-click
...
(`Shift+Enter` or `Ctrl+J` adds a line) and pacer launches `claude --cloud <task>`. On accounts without
Claude's live-attach rollout the CLI prints the session URL and exits — so pacer reads the session id off
that output and re-enters the session for you, without being asked. The row becomes a **mirror** of the
cloud session: pacer runs `claude --cloud <id>`, falling back to `claude --teleport <id>` (the transcript
...
a cloud transcript, and pacer stops respawning it under you. `NEBULA_CLOUD_MIRROR_SECS` changes the
...
editor — and pacer runs `claude -p <message> --cloud <id>` and pulls the transcript straight after. The
reply shows up on a later refresh; the CLI never returns one. Otherwise, name the session or accept the
default and pacer spawns the CLI in that worktree and drops you straight into it.
...
key to remember: the agent doesn't care that you stopped watching. Press `q` to quit pacer entirely and
the daemon still owns every PTY — come back with `pacer` an hour later and each session is where you
...
one project list gets long (each pacer instance keeps its own — run two, on two workspaces, side by
...
A dot going violet while you were looking elsewhere is easy to miss, so pacer counts those for you:
...
pacer finds the pull request open on each branch with `gh` and shows it in the Sessions panel's
...
- **Status via agent-CLI hooks, not MCP.** pacer merges managed hooks into each agent's config;
...
- **Sessions title themselves** after your first prompt (a name you type always wins), can move into
  a worktree on request ("do this in a worktree"), and can spawn siblings (`pacer spawn "<task>"`).
...
| Any panel | `Shift+H` | ssh hosts: every `pacer ssh` / `pacer tunnel` destination, newest first. `Enter`/click reconnects (quits this TUI and execs a fresh `pacer ssh` — local sessions keep running), `a` types a new `user@host [dir]`, `d` removes |
...
| Any panel | `Shift+M` | memory usage: RAM per agent/terminal process tree, pacer itself, and the machine-wide share; `↑/↓` + `Enter` opens the selected session |
...
pacer rename <title>     # title the current session (agents run this; --force to retitle)
pacer worktree [name] [--base <ref>]  # move the current session into a worktree of its project,
...
pacer spawn <task> [--kind <claude|codex|cursor>]  # start a new agent session beside the current
                          # one, in the same worktree, opening on <task> (agents run this when you
                          # ask for a new pacer session; --kind defaults to this session's harness)
...
pacer ssh <host> [dir]   # open pacer on a remote machine over ssh (installs it there if
                          # missing); destinations are remembered for the TUI's `h` picker
pacer tunnel <host> [dir] [--port N] [--remote-port N]
                          # that host's pacer in a browser tab here, over one ssh tunnel: installs
                          # pacer there if missing, runs `pacer browser` on its loopback, forwards
                          ...
                          # If that host already has a `pacer browser` on the port, the tunnel
                          ...
                          # local end (same rules as `pacer browser`), --remote-port the far end
...
pacer browser [--port N] [--bind ADDR | --public] [--credential USER:PASSWORD] [--no-open]
                          ...
                          # (0.0.0.0) — for a pacer on a remote box, where the access control
...
Workspace layout: `pacer-core` (shared protocol/entities), `pacer-daemon` (PTYs, SQLite, hook receiver,
status engine), `pacer-tui` (ratatui client), `pacer` (the binary). `vendor/vt100` is a patched copy of
