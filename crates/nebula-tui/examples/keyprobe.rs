...
//!
//! Run it in the terminal you launch nebula from and press the chord under
    ...
    loop {
        match event::read()? {
            Event::Key(key) => {
                ...
                let kitty = nebula_tui::keys::encode_key(&key, 5, false);
                let legacy = nebula_tui::keys::encode_key(&key, 0, false);
                write!(
                    out,
