...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    // pacer splash until the first project lands. Other empty workspaces
...
fn draw_overlay(f: &mut Frame, app: &mut App) {
    ...
    match overlay {
            ...
            const RIGHT: &[HelpSection] = &[
                ...
                (
                    ...
                    &[
                        ...
                        (Act(&[Metrics]), "memory usage (pacer + agents)"),
                        (Act(&[Splash]), "pacer splash (any key returns)"),
        ...
        Overlay::Settings(view) => {
            ...
            let path = pacer_core::paths::config_path();
            lines.push(Line::from(Span::styled(
        ...
        Overlay::Metrics(view) => {
            ...
            // can't pass for sessions; then pacer's own two processes.
            ...
            struct Row {
                ...
                procs: u32,
                bytes: u64,
                /// None = not openable: pacer's own processes, a group
            ...
            let wt_context = |wt_id: &pacer_core::WorktreeId| -> String {
                app.tree
                    .worktrees
                    .iter()
                    .find(|w| &w.id == wt_id)
            ...
            if let Some(snap) = &view.snapshot {
                ...
                rows.push(Row {
                    name: "pacer ui (this window)".into(),
                    context: String::new(),
            ...
            // re-sorts (sizes move rows around); pacer's own rows sit at
            ...
            if let Some(snap) = &view.snapshot {
                // Rollup: one line per agent kind, then pacer, then total.
                ...
                let pacer_bytes = snap.daemon_rss_bytes + view.client_rss_bytes;
                lines.push(Line::from(vec![
                    Span::styled(" pacer   ", header),
                    Span::styled(format!("{:<42}", "daemon + this ui"), dim),
                    Span::styled(format!("{:>9}", fmt_mem(pacer_bytes)), mem_style),
                ]));
                let total = sessions_total + pacer_bytes;
        ...
        Overlay::Palette(palette) => {
            ...
            for (row, (i, m)) in palette.matches.iter().enumerate().skip(start).enumerate() {
                ...
                let (solid, hollow) = match &item.target {
                    ...
                    // The panels' "leaves pacer" arrow: a pull request row
...
fn draw_session_row(
    ...
    let spans = match row {
        ...
        SessionRow::Link(l) => {
            ...
            // the column reads as one list. The arrow says "leaves pacer";
...
fn draw_footer_bar(f: &mut Frame, app: &App, area: Rect) -> Option<Rect> {
    ...
    let hints = if let Some(flash) = &app.flash {
        ...
        Span::styled(
            if view.input.is_some() {
                "type user@host [dir]  Enter: connect (restarts pacer over ssh)  Esc: cancel"
            } else {
                "↑/↓: select  Enter: connect (restarts pacer over ssh)  a: new  d: remove  Esc: close"
            },
            Style::default().fg(th.dim),
        )
    ...
    spans.push(hints);
    // Right edge: live session/process counts and pacer's total memory
    ...
    // Which pacer this is, at the far left: the one thing on the bar that
    ...
    let plate = format!("pacer v{}", env!("CARGO_PKG_VERSION"));
...
/// and pacer's total memory footprint (TUI + daemon + every session's
...
mod tests {
    ...
    fn hit_test_app(branches: &[&str], agents: &[&str], terminals: &[&str]) -> App {
        use pacer_core::{Agent, AgentId, AgentStatus, Project, ProjectId, Worktree, WorktreeId};
        let mut app = App::new();
        ...
        for (i, name) in terminals.iter().enumerate() {
            app.tree.terminals.push(pacer_core::TerminalTab {
                id: pacer_core::TerminalId(format!("t{i}")),
                worktree_id: WorktreeId("w0".into()),
                name: (*name).into(),
                sort_order: i as i64,
                alive: false,
