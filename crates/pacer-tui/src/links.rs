...
mod tests {
    ...
    fn finds_claude_style_path_with_line() {
        let files = screen_files(80, b"see crates/pacer-tui/src/ui.rs:1226 for it");
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, "crates/pacer-tui/src/ui.rs");
        assert_eq!(files[0].line, Some(1226));
        // The underline (and click target) covers the ":1226" suffix too.
        assert!(files[0].contains((4, 0)));
        assert!(files[0].contains((34, 0)));
    ...
    fn tool_header_parens_do_not_glue() {
        let files = screen_files(80, b"Read(crates/pacer/src/main.rs)");
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, "crates/pacer/src/main.rs");
    }
    ...
    fn wrapped_path_joins_across_rows() {
        ...
        let files = screen_files(20, b"in crates/pacer-tui/src/app.rs:5 x");
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].path, "crates/pacer-tui/src/app.rs");
