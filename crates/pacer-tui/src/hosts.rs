//! Recently-used `pacer ssh` destinations, backing the TUI's `h` picker.
//!
//! A plain JSON list in the data dir, most-recent first. `pacer ssh`
...
/// Parse a typed destination — `host` or `host dir` (the `pacer ssh`
...
fn store_path() -> PathBuf {
    ...
    pacer_core::paths::data_dir().join("ssh_hosts.json")
}
