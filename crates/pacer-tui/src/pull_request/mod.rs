...
//! refreshes on its own, so a PR opened outside pacer shows up without
...
pub struct PullRequest {
    ...
    /// compare — pacer never has to parse a date or trust a clock.
...
impl PullRequest {
    ...
    /// The mark to store when the user opens this PR: everything pacer
    ...
    /// never opened from pacer — leaves the whole conversation unread,
...
///   one most likely to have a pacer worktree still attached to it — the
...
pub struct PrDetail {
    ...
    /// pacer is not a markdown viewer, and mangling someone's fenced code
