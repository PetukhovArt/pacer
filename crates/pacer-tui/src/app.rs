...
pub struct DiffView {
    ...
    /// Pacer-side bookkeeping only (persisted via `review::store_marks`);
...
/// worktrees, then their sessions, then the open pull requests pacer has
...
/// Recent-hosts modal (`h`): destinations remembered by `pacer ssh`.
/// Enter (or a click) quits the TUI and execs a fresh `pacer ssh` at the
...
pub struct HostsView {
    ...
    /// `user@host [dir]`, Enter connects like a `pacer ssh` invocation.
...
/// Memory-usage modal (`M`): how much RAM pacer and every live session's
...
pub struct MetricsView {
    ...
    pub snapshot: Option<pacer_core::MetricsSnapshot>,
    ...
    /// mouse handlers agree with what's on screen. `None` = one of pacer's
...
/// or the pull request pacer found on the worktree's branch.
#[derive(Debug, Clone)]
pub enum LinkRow {
...
impl LinkRow {
    ...
    /// since it was last opened from pacer. Zero for a row that isn't a
...
pub struct Tree {
    ...
    /// hands over the last-opened one at boot, and after that every pacer
...
impl Tree {
    ...
    pub fn in_default_workspace(&self) -> bool {
        self.active_workspace.as_str() == pacer_core::DEFAULT_WORKSPACE_ID
    }
...
/// Pacer owns the mouse (the emulator's native shift+drag never reaches us
...
pub struct App {
    ...
    pub last_metrics: Option<pacer_core::MetricsSnapshot>,
...
impl App {
    pub fn new() -> Self {
        Self {
            ...
            hostname: pacer_core::host::hostname(),
            is_remote: pacer_core::host::is_remote_session(),
    ...
    /// armed. A project pacer has never asked about is always due.
...
mod tests {
    ...
    /// A link the user pasted before pacer found the PR is the same
