...
pub enum ClientRequest {
    ...
    /// fresh client opens into. Deliberately not broadcast: two pacer
    ...
    /// scoped itself, which is every one-shot `pacer add`.
    AddProject {
        req_id: u64,
        path: PathBuf,
        name: Option<String>,
    ...
    CreateAgent {
        ...
        /// runs `pacer rename` on its first prompt).
        auto_title: bool,
    ...
    /// `pacer worktree <name>`, run by the agent from inside its own
    ...
    /// `pacer spawn "<task>"`, run by the agent from inside its own
...
/// lexicographically, so "arrived since" is a string compare and pacer
