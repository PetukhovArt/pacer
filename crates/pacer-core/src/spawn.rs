...
//! `DETACHED_PROCESS`, see `pacer-tui`'s `ipc::detach`) must keep setting
//! its own, and interactive handoffs (`pacer ssh`, ttyd) must keep the
...
/// through the user's login shell (see `pacer-daemon`'s `launch`).
#[cfg(windows)]
mod windows_resolve {
    ...
    /// carry. pacer's audience always has Git for Windows, and it ships
    ...
    mod tests {
        ...
        fn path_is_walked_in_order_for_a_bare_name() {
            let tmp = tempfile::tempdir().unwrap();
            let dir = tmp.path();
            std::fs::write(dir.join("pacer-probe-stub.exe"), b"").unwrap();
            let previous = std::env::var_os("PATH");
            std::env::set_var("PATH", dir);
            let found = resolve_program("pacer-probe-stub");
            match previous {
