...
pub struct Daemon {
    ...
    /// pacer hasn't adopted yet leaves its cwd here, so the worktree sync
    /// can finish the re-home once the row exists.
    last_cwd: Mutex<HashMap<AgentId, PathBuf>>,
    /// Agents that ran `pacer worktree` and are waiting for their turn to
    ...
    /// pacer picks it up without anyone clearing a flag.
...
impl Daemon {
    ...
    /// `pacer add`, or a client still on whatever it booted into — means
    ...
    pub async fn add_project(
        ...
        // worktrees too so `pacer` matches reality on day one. Root-ness is
    ...
    pub fn remove_project(self: &Arc<Self>, id: &ProjectId) -> Result<()> {
        ...
        // Removing a project only forgets it in pacer — never touches disk.
    ...
    /// checkouts made outside pacer (an agent running `git worktree add`,
    ...
    /// Agent-initiated one-shot title (`pacer rename` inside the session's
        ...
        if !self.store.rename_agent_if_auto_pending(id, &title)? {
            bail!(
                "session already has a title ({:?}); leaving it unchanged — a user-set \
                 title is only replaced with `pacer rename --force`",
                agent.name
            );
        }
        self.broadcast_agent(id)?;
    ...
    /// `pacer worktree <branch>`, run by the agent inside its own session.
    ...
    /// The turn an agent ran `pacer worktree` in has ended: make the
    ...
    fn try_reparent_agent_by_cwd(
        ...
        // `git worktree add` and stepped into the result reports a cwd pacer
    ...
    /// Queue a message on a Cloud session without leaving pacer.
    ...
    /// (the relocation notice a `pacer worktree` respawn opens with, or the
    ...
    fn spawn_agent_session_with(
        ...
        let install_result = match agent.kind {
            ...
            // any per-worktree copy an older pacer left is pruned.
            AgentKind::Codex => {
...
/// argument, and an `initial_prompt` — the relocation notice a `pacer
...
/// What pacer appends to Claude's system prompt: how to take a "do this
/// in a worktree" request through pacer (`Daemon::enter_worktree`) instead
/// of Claude's own EnterWorktree tool, whose checkout lands under
/// `<repo>/.claude/worktrees/` on a `worktree-*` branch — a layout the
/// worktree list only adopts after the fact, and not where a pacer user
/// keeps their worktrees. Claude only: codex and cursor have no
/// system-prompt flag, and no EnterWorktree to steer away from.
pub const CLAUDE_WORKTREE_GUIDANCE: &str = "[pacer] This session runs inside pacer, which \
...
short kebab-case name for the task (`pacer worktree` with no name invents one; `--base <ref>` picks \
the start point). pacer creates the worktree, associates this session with it, and relocates the \
...
fn claude_pr_system_prompt(pr_url: &str) -> String {
    format!(
        "[pacer] This session was created from the OPEN PRS row for {pr_url}. All work in this \
...
/// sandbox has no pacer CLI to follow it with. Values bind with `=`
...
mod tests {
    ...
    /// Claude argv: `args`, then pacer's appended guidance (worktree and
    ...
    fn spawn_command_keeps_pr_scope_and_url_in_claudes_system_prompt() {
        let pr_url = "https://github.com/PetukhovArt/pacer/pull/42";
    ...
    fn move_agent_rehomes_row_and_broadcasts() {
        let daemon = test_daemon();
        seed_projects(&daemon, &["p"]);
        seed_worktree(&daemon, "p", "root", "/pacer-test/p", true);
        seed_worktree(&daemon, "p", "feat", "/pacer-test/p-feat", false);
    ...
    /// Between `pacer worktree` and the turn's Stop the row already sits
    ...
    fn pending_relocation_ignores_the_old_cwd_until_the_turn_ends() {
        ...
        daemon.reparent_agent_by_cwd(&a1, "/pacer-test/p", Some("s1"), false);
        assert_eq!(
    ...
    #[tokio::test]
    async fn enter_worktree_creates_the_checkout_in_pacers_layout() {
    ...
    fn move_agent_rejects_cross_project_targets() {
        ...
        seed_worktree(&daemon, "p", "p-root", "/pacer-test/p", true);
        seed_worktree(&daemon, "q", "q-root", "/pacer-test/q", true);
    ...
    fn cwd_replay_skips_other_projects_and_archived_agents() {
        ...
        seed_worktree(&daemon, "p", "p-root", "/pacer-test/p", true);
        seed_worktree(&daemon, "q", "q-root", "/pacer-test/q", true);
        seed_worktree(&daemon, "q", "q-feat", "/pacer-test/q-feat", false);
        ...
        daemon
            ...
            .unwrap();
        daemon.reparent_agent_by_cwd(&AgentId("a1".into()), "/pacer-test/q-feat", None, false);
        daemon.reparent_agent_by_cwd(&AgentId("a2".into()), "/pacer-test/q-feat", None, false);
        seed_worktree(&daemon, "q", "q-feat", "/pacer-test/q-feat", false);
    ...
    /// `pacer add .` from inside a linked worktree used to make the worktree
    ...
    fn cli_missing_message_names_the_binary_not_the_kind() {
        ...
        // No "restart pacer": agent CLIs are spawned through the user's
    ...
    fn reparent_by_cwd_skips_archived_agents() {
        ...
        daemon.reparent_agent_by_cwd(&AgentId("a1".into()), "/pacer-test/p-feat", None, false);
        assert_eq!(agent_worktree(&daemon, "a1"), "root");
    }
