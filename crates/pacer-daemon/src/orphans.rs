...
//! but it only knows what pacer copied aside before the delete cascade, so
...
use anyhow::Result;
use pacer_core::{paths, OrphanedSession, Project, Worktree};
...
fn claude_projects_dir() -> Option<PathBuf> {
    pacer_core::env::home_dir().map(|home| home.join(".claude").join("projects"))
}
...
/// checkout, and the WORKTREE DIR beside it where pacer puts new worktrees
/// (`<repo>/../<repo-name>-worktrees`).
///
/// This is what keeps one project's transcripts out of another's list. The
/// slug prefix alone cannot: `…/pacer` is a prefix of `…/pacer2`, whose
...
fn scan_claude_transcripts(
    ...
    // narrows it: `…/pacer` is a prefix of `…/pacer2` as well, so where a
...
mod tests {
    ...
    /// The slug is a data format owned by another program: pacer only
    ...
    fn slug_matches_claude_codes_directory_names() {
        assert_eq!(
            slug(Path::new(r"D:\web-projects\pacer")),
            "D--web-projects-pacer"
        );
        assert_eq!(
            slug(Path::new(r"D:\web-projects\pacer-worktrees\features")),
            "D--web-projects-pacer-worktrees-features"
        );
    ...
    fn a_worktrees_slug_starts_with_its_projects() {
        let project = slug(Path::new(r"D:\web-projects\pacer"));
        let worktree = slug(Path::new(r"D:\web-projects\pacer-worktrees\features"));
    ...
    fn project_at(repo: &Path) -> Project {
        Project {
            ...
            id: pacer_core::ProjectId("p".into()),
            name: "demo".into(),
    ...
    fn worktree_at(path: &Path) -> Worktree {
        Worktree {
            id: pacer_core::WorktreeId("w".into()),
            project_id: pacer_core::ProjectId("p".into()),
...
",
    ...
    fn a_session_known_to_both_sources_is_one_row() {
        ...
        from_disk.kind = pacer_core::AgentKind::Claude;
        ...
        assert_eq!(merged[0].kind, pacer_core::AgentKind::Codex);
