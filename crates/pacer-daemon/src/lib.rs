...
async fn serve() -> Result<()> {
    let Some(_lock) = lifecycle::PidfileLock::try_acquire()? else {
        bail!("another pacer daemon is already running");
    };
    // Record which build this daemon runs so installers can tell an
    // up-to-date daemon from a stale one (`pacer _stale-daemon-note`).
    ...
    tracing::info!(pid = std::process::id(), socket = %endpoint, "pacer daemon listening");
    ...
    // cwd inside another worktree of the same project re-homes the agent row.
    {
        ...
        tokio::spawn(async move {
            ...
            }) = hook_rx.recv().await
            {
                ...
                // A `pacer worktree` relocation waits for the turn to end.
    ...
    // Worktrees created, removed, or re-checked-out outside pacer (an agent
    ...
    // first tick also runs one boot-time sync per project.
    {
        ...
        tokio::spawn(async move {
            ...
            let mut seen: std::collections::HashMap<pacer_core::ProjectId, std::time::SystemTime> =
...
/// and the DETACHED_PROCESS launch (see `pacer-tui`'s `spawn_daemon`) gives
/// the daemon its own console, so neither reaches it from the TUI's terminal
/// — they arrive only from a deliberate `taskkill` or a `pacer kill`
...
/// what lets a project rooted at a worktree (one added with `pacer add` from
