...
pub const AUTO_TITLE_INSTRUCTION: &str = "[pacer] Before addressing the \
user's request, run this shell command exactly once:\n\n  pacer rename \
<title>\n\nReplace <title> with 3-4 Title Case words describing the user's \
request, unquoted (example: pacer rename Fix Login Redirect). If it reports \
...
mod tests {
    use super::*;
    use pacer_core::{Agent, AgentKind, AgentStatus, Project, ProjectId, Worktree, WorktreeId};
    ...
    async fn user_prompt_submit_injects_title_instruction_only_while_pending() {
        ...
        assert!(body.contains("pacer rename"), "instruction: {body}");
