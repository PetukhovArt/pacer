//! `pacer spawn "<task>"` from inside an agent session: a new AGENT beside
...
//! never touched (unlike `pacer worktree`, nothing here waits on a turn
//! end), so the model runs it, tells the user, and carries on.

use std::sync::Arc;

use anyhow::{bail, Context, Result};
use pacer_core::{AgentId, AgentKind, EntityId};

use crate::registry::{CreateAgentSpec, Daemon};

/// What pacer appends to Claude's system prompt so "start a new pacer
/// session that …" becomes one `pacer spawn` call instead of the model
...
pub const CLAUDE_SPAWN_GUIDANCE: &str = "[pacer] When the user asks you to start a new pacer \
session (\"start a new pacer session that …\", \"spin up another session to …\", \"open a new \
...
matches this one. pacer starts it beside this session, in the same worktree, and it shows up in \
...
impl Daemon {
    ...
    /// `pacer spawn`, run by the agent inside its own session: create and
...
mod tests {
    ...
    use pacer_core::{Agent, AgentStatus, Project, ProjectId, Worktree, WorktreeId};
        ...
        for (id, is_main) in [("root", true), ("feat", false)] {
            daemon
                ...
                .insert_worktree(&Worktree {
                    ...
                    path: format!("/pacer-test/p-{id}").into(),
                    branch: id.into(),
                    is_main,
                    sort_order: 0,
                })
