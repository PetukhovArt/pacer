...
//! are the transport's business (`pacer_core::transport`) — this module
...
use pacer_core::transport::{Authorizer, Listener, Stream};
use pacer_core::{ClientRequest, ServerEvent, SessionRef, WorkspaceId, PROTOCOL_VERSION};
...
async fn handle_client(
    ...
    // two pacer instances are two independent views, so one switching
    ...
    // `pacer add`, whose workspace genuinely is the current default.
            ...
            match req {
                ...
                ClientRequest::CreateAgent {
                    ...
                    if let Some(launch_mode) = launch_mode {
                        match &result {
                            Ok(pacer_core::EntityId::Agent(agent)) => tracing::info!(
                                req_id,
                                agent = %agent,
                                kind = kind.as_str(),
                                worktree = %worktree,
...
async fn forward_pty(
    ...
    loop {
        match rx.recv().await {
            ...
            // Daemon-side only: the progress and title edges drive the
            // status machine and reach clients as a StatusChanged, not as
            // session output; the cloud sightings reach them as the row's
            // own upsert.
            Ok(
                PtyEvent::Progress { .. }
                | PtyEvent::TitleIdle { .. }
...
async fn reply(
    out_tx: &mpsc::Sender<ServerEvent>,
    req_id: u64,
    result: anyhow::Result<Option<pacer_core::EntityId>>,
) {
    let ev = match result {
