...
pub const GIT_MISSING: &str =
    "git was not found on your PATH — pacer needs it. Install git (https://git-scm.com/downloads), then restart pacer.";
...
mod tests {
    ...
    async fn a_paused_rebase_keeps_the_worktree_on_its_branch() {
        ...
        let wt_canon = pacer_core::paths::canonical_or_raw(&wt);
        let branch_of = |entries: &[WorktreeEntry]| {
            entries
                .iter()
                .find(|e| pacer_core::paths::canonical_or_raw(&e.path) == wt_canon)
