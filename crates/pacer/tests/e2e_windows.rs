...
//! *replaced*, run against the real `pacer` binary and a real daemon.
//!
...
//! launch is gated on pacer answering the host's `ESC[6n` (see
//! `pacer_core::dsr`), and `pacer-daemon`'s and `pacer-tui`'s own PTY
//! tests run the full child lifecycle on Windows.

#![cfg(windows)]

use pacer_core::codec::{read_frame, write_frame};
use pacer_core::{env, ClientRequest, ServerEvent, PROTOCOL_VERSION};
...
impl TestEnv {
    ...
    /// The `pacer` binary under test, pointed at this env's runtime and data
...
/// `pacer daemon --foreground` that nothing reaps: it is DETACHED_PROCESS,
...
/// `pacer_core::transport::connect` does.
...
/// `pacer-tui` cannot depend on `pacer-daemon`, so each spells the PIDFILE
...
/// and `pacer kill` would silently do nothing.
#[test]
fn both_sides_lock_the_same_pidfile_byte() {
    ...
    assert_eq!(
        pacer_daemon::lifecycle::LOCK_OFFSET,
        0x4000_0000,
        "if this moved, move pacer-tui's PIDFILE_LOCK_OFFSET with it"
    );
}
...
async fn the_auto_spawned_daemon_outlives_the_client_that_started_it() {
    ...
    // `pacer add` on a real directory: a one-shot client that spawns the
