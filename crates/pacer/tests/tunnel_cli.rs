...
fn tunnel(args: &[&str]) -> std::process::Output {
    ...
    Command::new(env!("CARGO_BIN_EXE_pacer"))
        .arg("tunnel")
        .args(args)
        .env("PATH", "")
        .env(pacer_core::env::DATA_DIR, data.path())
        .output()
        .expect("failed to run pacer tunnel")
}
...
/// a connection that already installed pacer on the far end.
#[test]
