//! True end-to-end TUI test: runs the real `pacer` binary inside a PTY,
...
impl TuiHarness {
    ...
    fn spawn_with_env(extra_env: &[(&str, String)]) -> Self {
        ...
        let pty = native_pty_system()
            ...
            .unwrap();
        let mut cmd = CommandBuilder::new(env!("CARGO_BIN_EXE_pacer"));
        cmd.env(pacer_core::env::RUNTIME_DIR, &runtime_dir);
        cmd.env(pacer_core::env::DATA_DIR, &data_dir);
        cmd.env(pacer_core::env::AGENT_CMD, "/bin/sh"); // stand-in for claude
        cmd.env(pacer_core::env::WORKTREE_SYNC_MS, "100"); // fast external-change pickup
        cmd.env(pacer_core::env::LOG, "debug");
    ...
    fn make_repo(&self, name: &str) -> PathBuf {
        ...
        git(&["init", "-b", "main"]);
        git(&["config", "user.email", "t@pacer.dev"]);
...
impl Drop for TuiHarness {
    fn drop(&mut self) {
        ...
        let _ = std::process::Command::new(env!("CARGO_BIN_EXE_pacer"))
            .arg("kill")
            .env(pacer_core::env::RUNTIME_DIR, &self.runtime_dir)
            .env(pacer_core::env::DATA_DIR, &self.data_dir)
...
/// (mirrors DEFAULT_PANEL_WIDTHS in pacer-tui/src/app.rs; the harness
...
/// The pull request pacer finds on the branch leads the OPEN PRS group. A
