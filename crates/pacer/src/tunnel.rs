//! `pacer tunnel HOST [PATH] [--port N] [--remote-port N]`: run the remote
//! machine's pacer in a browser tab here, with nothing exposed on the
//! remote's network.
//!
...
//! `pacer browser --no-open --port <remote>`, which stays on the remote's
...
//!
//! If the remote already has a ttyd on that port — a `pacer browser` the
...
//!
//! We spawn ssh rather than exec it (unlike `pacer ssh`) because there is
...
//! both directions — Ctrl+C here reaches the remote pacer and ttyd, and
...
/// a pacer release before ttyd is even spawned.
const STARTUP_TIMEOUT: Duration = Duration::from_secs(120);
/// Slower than `pacer browser`'s poll on purpose: until the far end is up,
...
macro_rules! reuse_existing_ttyd {
    () => {
        concat!(
            ...
            "echo \"pacer tunnel: a pacer browser is already serving on this host at port $2; reusing it ",
...
///
/// stdout is dropped because the remote `pacer browser` addresses a user
...
/// behind `--credential` included). If so, this is a `pacer browser` the
...
/// hang-up ends the sleep exactly as it would have ended `pacer browser`.
/// The probe is shell-only, so a remote whose pacer predates this still
/// reuses; a remote with no `curl` skips the probe and behaves as before.
///
/// The `--help` grep is the version check. `pacer ssh` and this only install
/// pacer when the remote has *none*, so a box last touched a few releases
/// ago keeps a `pacer browser` that predates `--no-open` and would fail on
/// an unknown argument. Naming the fix beats a clap usage dump.
const REMOTE_SCRIPT: &str = concat!(
    install_prelude!(),
    reuse_existing_ttyd!(),
    "pacer browser --help 2>/dev/null | grep -q -- --no-open || { ",
    "echo \"pacer tunnel: the pacer on this host is too old to tunnel into; ",
    "reach it with pacer ssh and run pacer upgrade there\" >&2; exit 1; }; ",
    "cd -- \"${3:-$HOME}\" || exit 1; ",
    "exec pacer browser --no-open --port \"$2\" >/dev/null"
);

/// Everything `pacer tunnel` was asked for, straight off the CLI.
#[derive(Debug, Clone)]
pub struct TunnelOpts {
    /// ssh destination, passed to ssh verbatim (e.g. `user@10.0.1.7`).
    pub host: String,
    /// Remote directory the served pacer starts in.
...
pub fn run_tunnel(opts: TunnelOpts) -> Result<()> {
    ...
    // The same list `pacer ssh` writes and the TUI's `h` picker reads — a
    ...
    pacer_tui::hosts::record(&opts.host, opts.path.as_deref());

    println!(
        "pacer tunnel: connecting to {} (localhost:{local} → its 127.0.0.1:{remote})",
        opts.host
    );
    ...
    if browser::open_url(&url) {
        println!("pacer tunnel: {} is serving on {url}", opts.host);
    } else {
        println!(
            "pacer tunnel: {} is serving on {url} (open it yourself — no browser launched)",
            opts.host
        );
    }
    ...
    match status.code() {
        // 130 is the remote pacer taking this terminal's Ctrl+C through the
...
///
/// Mirrors `pacer browser`'s rules — [`browser::DEFAULT_PORT`] when free, a
/// named port or an error, `0` for any — because from the browser's side this
/// *is* a `pacer browser`, just one whose terminal lives on another machine.
fn resolve_local_port(requested: Option<u16>) -> Result<u16> {
    match requested {
        ...
        None => {
            ...
            println!(
                "pacer tunnel: {} is busy here — forwarding {port} instead",
...
fn spawn_ssh(opts: &TunnelOpts, local: u16, remote: u16) -> Result<Child> {
    ...
    Command::new("ssh")
        ...
        // remote pacer and its ttyd, and hanging up kills them rather than
        ...
        .map_err(|e| match e.kind() {
            std::io::ErrorKind::NotFound => {
                anyhow!("ssh not found on PATH — pacer tunnel requires the OpenSSH client")
            }
...
/// installs pacer first, that is hundreds of lines burying the messages that
...
fn wait_until_forwarded(child: &mut Child, addr: SocketAddr, host: &str) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            let _ = child.kill();
            bail!(
                "{host} did not start serving within {}s — connect with `pacer ssh {host}` and check that ttyd is installed there",
...
mod tests {
    ...
    /// The same constraint `pacer ssh`'s script lives under: it is wrapped
    ...
    fn the_remote_serves_without_opening_anything() {
        ...
        assert!(cmd.contains("pacer browser --no-open --port"), "{cmd}");
    ...
    /// rather than left to `pacer browser`'s free-port fallback.
    #[test]
    fn the_port_reaches_the_script_as_a_parameter() {
        let cmd = remote_command(URL, 9123, None);
        assert!(cmd.contains("pacer-tunnel 'https://example.com/install.sh' 9123"));
    ...
    /// A ttyd already on the remote port is a `pacer browser` the user has
    ...
    fn an_existing_ttyd_on_the_remote_port_is_reused_before_anything_starts() {
        ...
        let start = REMOTE_SCRIPT.find("exec pacer browser").unwrap();
    ...
    /// `$HOME/.local/bin`, and the real pacer must not be found) and a
    /// stub `pacer` first on PATH that answers nothing — so the version
    ...
    fn run_remote_script(port: u16) -> (std::process::Child, tempfile::TempDir) {
        ...
        let pacer = stub.join("pacer");
        std::fs::write(&pacer, "#!/bin/sh\nexit 0\n").unwrap();
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            std::fs::set_permissions(&pacer, std::fs::Permissions::from_mode(0o755)).unwrap();
        }
        let path = format!(
        ...
        let child = Command::new("sh")
            .args(["-c", REMOTE_SCRIPT, "pacer-tunnel", "file:///nonexistent"])
            .arg(port.to_string())
            .env("HOME", home.path())
            .env("PATH", path)
            .stdin(Stdio::null())
    ...
    /// is a Windows path, the stub `pacer` carries no exec bit, and `/usr/bin`
    ...
    /// start its own — here, into the version gate the stub pacer fails.
    /// Stands [`REMOTE_SCRIPT`] up under a local `sh` to check what the
    /// *remote* does with it. Unix-only, and not because the remote could be
    /// Windows — it cannot; because only a Unix host can stand in for one.
    /// Under MSYS `sh` the simulation drifts from the thing simulated: `$HOME`
    ...
    fn a_remote_too_old_to_tunnel_is_named_as_such() {
        ...
        assert!(REMOTE_SCRIPT.contains("pacer upgrade"));
    }
    ...
    /// is what makes the printed URL readable as "that host's pacer".
    #[test]
    ...
    /// `pacer browser`) already holds it. Deliberately does not stand on
