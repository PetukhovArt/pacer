//! `pacer upgrade`: re-run the published install script.
//!
...
pub fn run_upgrade(force: bool) -> Result<()> {
    ...
    pacer_daemon::lifecycle::ensure_runtime_dir()?;
    upgrade_with(&url, &pacer_core::paths::runtime_dir(), force)?;
...
fn finish_daemon_handoff() {
    use pacer_tui::ipc::IdleShutdown;
    match pacer_tui::shutdown_daemon_if_idle() {
...
/// resolve first: the usual dev setup is a `~/.cargo/bin/pacer` symlink
