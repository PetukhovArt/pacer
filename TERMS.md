...
| **DAEMON SOCKET** | What the TUI and one-shot CLI clients connect to. On Unix `<RUNTIME DIR>/daemon.sock`, where the dir's mode 0700 is the authorization; on Windows a loopback TCP listener plus a bearer token, both published in the ENDPOINT FILE and presented before `Hello`. Callers see one `Stream` either way. | "the socket" | `nebula-core/src/transport.rs` · `paths.rs::socket_path` |
| **RUNTIME DIR** | DAEMON SOCKET + PIDFILE directory: `$NEBULA_RUNTIME_DIR`, else `$XDG_RUNTIME_DIR/nebula`, else `/tmp/nebula-<uid>` — on Windows `%TEMP%\nebula`, which needs no explicit mode because the profile's inherited ACL already draws the same boundary 0700 does. | — | `paths.rs::runtime_dir` |
...
| **PIDFILE LOCK** | An exclusive, non-blocking lock on `<RUNTIME DIR>/daemon.pid` is daemon liveness; clients probe it by trying the lock. `flock` on Unix, `LockFileEx` on Windows — where it is taken on a byte 1 GiB in, because a Windows lock is *mandatory* and a locked byte 0 would make the pidfile unreadable to the very clients that read it. | "pidfile" | `nebula-daemon/src/lifecycle.rs::PidfileLock`, `try_lock_exclusive` |
...
| **PTY SESSION** | The DAEMON-owned PTY child for one SESSION: reader thread, output coalescing, a polite hangup then a whole-process-group kill (`killpg` on Unix, a Job Object on Windows), plus the PROGRESS SCANNER, KITTY SCANNER and CLOUD SCANNER on its stream. | "the pty" | `nebula-daemon/src/pty/mod.rs::PtySession` |
...
| **DAEMON SETSID** | The TUI spawns the DAEMON with `setsid` (a new *session*, not just a process group) so it has no controlling tty: a daemon child that could reach `/dev/tty` would write into the TUI. Windows spells it `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP` — deliberately not `CREATE_NO_WINDOW`, since the point is no console *at all*. | "keeps writing strange tokens and the entire app is broken" | `nebula-tui/src/ipc.rs::spawn_daemon`, `detach` |
...
| **CLI PROBE** | Boot-time "is this CLI installed" per AGENT KIND, asked the way the CLI will be launched (`command -v` through the login shell on Unix, a PATH × PATHEXT lookup on Windows), cached (1 h ok / 60 s fail); a probe that cannot answer reports available and caches nothing. Skipped under `NEBULA_AGENT_CMD`. | — | `registry.rs::warm_cli_probes` · `launch.rs::program_is_installed` |
| **LOGIN SHELL WRAP** | Agents are spawned through `$SHELL -l -i -c`, so the user's rc files (and their PATH prepends) apply. Windows has no equivalent and needs none — one registry-held PATH, so the CLI is launched directly, resolved over PATH × PATHEXT, with `.cmd` / `.bat` npm shims handed to `cmd.exe`. | "new sessions don't use my ~/.zshrc" | `nebula-daemon/src/launch.rs::wrap_for_user_env` |
...
| **NEBULA UPGRADE** | Run INSTALL.SH over this binary; refuses a local cargo build without `--force`; a running daemon keeps its old binary until NEBULA KILL. Unix-only — it wraps a `sh` script fetching release assets that are not published for Windows, so there it refuses and names `cargo install --path crates/nebula` instead. | "upgrade", "update nebula" | `crates/nebula/src/upgrade.rs` · `nebula upgrade [--force]` |
...
| **E2E PTY** | The daemon end-to-end suite over the real IPC: CRUD, attach/scrollback, hooks, progress, CWD REPARENT, cloud chain, restart persistence. `#![cfg(unix)]` in this fork — it asserts the AF_UNIX DAEMON SOCKET, `#!/bin/sh` stubs and `chmod` bits — so it stays the gate on a Unix host. Flaky on a cold binary ("daemon socket never appeared") — rerun before debugging. | "e2e_pty", "the e2e tests" | `crates/nebula/tests/e2e_pty.rs` |
...
| **E2E TUI** | Runs the real `nebula` binary in a PTY, sends raw key bytes (`\x1b[Z` = Shift+Tab, `\x1b[108;6u` = kitty Ctrl+Shift+L), parses frames with vt100. "The walk stops here" is proved by pressing again and walking back. `#![cfg(unix)]` in this fork, like E2E PTY. | "e2e_tui" | `crates/nebula/tests/e2e_tui.rs` |
...
| **ENDPOINT FILE** | `<RUNTIME DIR>/daemon.endpoint`, written by the DAEMON on Windows only: the loopback port and the per-boot bearer token, one per line, that a client reads to reach the DAEMON SOCKET. Removed on clean shutdown so the next client does not chase a dead port. | 2026-08-30 MEMORY "The Workspace Builds And Tests On Windows; ConPTY Blocks Every PTY SESSION" | `nebula-core/src/paths.rs::endpoint_path` · `transport.rs::write_endpoint_file` |
| **CONPTY BLOCKER** | On this Windows machine `portable-pty` 0.9 opens the pseudo console but the child spawned into it never runs — no output, then a hang or `0xC0000142`. It gates every PTY SESSION, and because a child that never runs never exits, it makes the *test binary hang at exit* rather than fail. | 2026-08-30 MEMORY "The Workspace Builds And Tests On Windows; ConPTY Blocks Every PTY SESSION" | `crates/nebula-tui/src/editor_stub.rs` (note at end) |
| **E2E WINDOWS** | The Windows smoke grid: end-to-end cover for what the port replaced and upstream therefore has no tests for — the loopback DAEMON SOCKET, the token in both verdicts, the ENDPOINT FILE, the PIDFILE LOCK refusing a second DAEMON, DETACHED_PROCESS survival. Complement to E2E PTY / E2E TUI, not a replacement. | 2026-08-30 prompt ("Windows-смоук ~10 тестов") · 2026-08-30 MEMORY "The Workspace Builds And Tests On Windows; ConPTY Blocks Every PTY SESSION" | `crates/nebula/tests/e2e_windows.rs` |
